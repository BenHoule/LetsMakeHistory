// Roll functions for determining triggers, sizes, and directions
import type { SourceType, SizeCategory } from '@lmh/types';
import { makeRNG } from './rng.js';

export const TRIGGER_CHANCE: Record<SourceType, number> = {
  EVENT:         0.30,
  COURT:         0.20,
  NPC_BILL:      0.30,
...
*/
export function rollTrigger(source: SourceType, rng: () => number): boolean {
 ...
 */
export function rollSize(rng: () => number): SizeCategory {
  const roll = rng();
 ...
 */
export function rollDirection(rng: () => number): 1 | -1 {
  return rng() < 0.50 ? 1 : -1;
}
