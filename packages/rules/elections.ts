import type {Party, Region } from '@lmh/types';

interface RegionPartyApproval {
  party: Party;
  partyApproval: number;
  regionalModifier: number;
}

function weightedApproval(r: RegionPartyApproval): number {
 ...
 */
export function resolveSenateClass(
  regionParties: RegionPartyApproval[],
  player?: { party: Party, approval: number },
): Party {
  // Sort the regional party approvals by their weighted approval to determine the leading party.
  const sorted = [...regionParties].sort((a, b) =>
    weightedApproval(b) - weightedApproval(a),
  );
  ...
  if (player && player.party !== leader.party) {
    ...
    if (buffer > gap) return player.party;
  }

  return leader.party;
}

interface NomineeRegionStats {
  partyApproval: number;
  regionalModifier: number;
}
...
export function resolvePresidentialElection(
  nomineeA: { party: Party; regions: NomineeRegionStats[] },
  nomineeB: { party: Party; regions: NomineeRegionStats[] },
): { winner: Party; electoralVote: [number, number]; isTieBreak: boolean } {
  ...
  for (let i = 0; i < nomineeA.regions.length; i++) {
    ...
    if (aWeighted >= bWeighted) aElectors++;
    else bElectors++;
  }
