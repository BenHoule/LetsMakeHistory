...
export function updateActionEdit(
	actionId: string,
	field: 'editedType' | 'editedContent',
	value: string
) {
	updActions((a) => a.map((r) => (r.actionId === actionId ? { ...r, [field]: value } : r)));
}
...
export function updateStatRoll(
	actionId: string,
	patch: Partial<
		Pick<StatRollRecord, 'stat' | 'target' | 'reason' | 'sizeCategory' | 'directionHint'>
	>
) {
...
export function setVisibility(s: VisibilitySettings) {
	setVisStore(s);
}
