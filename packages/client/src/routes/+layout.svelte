...
{#if guideOpen}
	<div
		...
		onkeydown={(e) => {
			if (e.key === 'Enter' || e.key === ' ') guideOpen = false;
		}}
	>
		<div
			role="dialog"
		...
		>
			<div
				style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:16px 18px;border-bottom:1px solid #d8d0b8;background:#f1ecdf;"
			>
				<div>
					<h2 style="font-family:Georgia,serif;font-size:22px;margin:0;">Rules Guide</h2>
					<p
						style="margin:4px 0 0;color:#7a7362;font-size:12px;text-transform:uppercase;letter-spacing:0.06em;"
					>
						Guide v2.1
					</p>
				</div>
				...
				>
					<button
						...
						style="padding:7px 11px;background:#1f3a5f;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:12px;font-weight:600;"
						>Open Raw</button
					>
					<button
						onclick={() => (guideOpen = false)}
						style="padding:7px 11px;background:#faf7f0;border:1px solid #232019;border-radius:4px;cursor:pointer;font-size:12px;font-weight:600;"
						>Close</button
					>
				</div>
			</div>
