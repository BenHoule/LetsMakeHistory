<script lang="ts">
	...
	function onJoined(playerId: string) {
		goto(resolve('/session/[id]', { id: sessionId + `?player=${encodeURIComponent(playerId)}`}));
	}
</script>
...
<main class="max-w-lg mx-auto p-8 space-y-6">
	...
	{#if !sessionId}
		<section class="border rounded p-4 space-y-2">
			<h2 class="font-semibold">
				Create a new session <span class="text-xs text-gray-400 font-normal">(GM)</span>
			</h2>
			<input
		...
		<section class="border rounded p-4 space-y-2">
			<h2 class="font-semibold">
				Join an existing session <span class="text-xs text-gray-400 font-normal">(Player)</span>
			</h2>
			<p class="text-xs text-gray-500">
				Ask your GM for the session ID, or use the share link they sent you.
			</p>
			<input
	...
	{:else}
		<div class="border rounded p-4 space-y-1 bg-gray-50">
			...
			{#if gmToken}
				...
				<p class="text-xs text-gray-400">
					GM dashboard: <a href={resolve(`/gm/${sessionId}`)} class="underline text-blue-600"
						>/gm/{sessionId}</a
					>
				</p>
			{/if}
		...
		<PlayerJoinForm {sessionId} {onJoined} />
	{/if}
</main>
