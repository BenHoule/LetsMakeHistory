<script lang="ts">
	...
	import type { PlayerState, SessionMeta, SessionStateResponse, TurnState } from '@lmh/types';
	import { statNotifications } from '../../../stores/notifications.js';

	const sessionId = page.params.id!;
	type SessionStoreWithPlayer = {
		_playerId?: string;
		players?: PlayerState[];
		session?: SessionMeta | null;
	};
	type NationalStatusPayload = {
		financial?: string;
		social?: string;
		foreign?: string;
	};
	type SessionWithServerFields = SessionMeta & {
		next_senate_class?: number;
		national_status?: string | NationalStatusPayload | null;
	};

	// Derived: find this player's senator info
	let myPlayer = $derived(
		($sessionStore as SessionStoreWithPlayer).players?.find(
			(p) => p.id === ($sessionStore as SessionStoreWithPlayer)._playerId
		)
	);
	...
	let nextSenateClass = $derived(
		(($sessionStore as SessionStoreWithPlayer).session as SessionWithServerFields | undefined)
			?.next_senate_class ?? 1
	);
	...
	let electionAlert = $derived(() => {
		...
		if (presCountdown <= 2)
			alerts.push(
				`Presidential election in ${presCountdown} turn${presCountdown === 1 ? '' : 's'}`
			);
		...
		for (const cls of [1, 2, 3] as const) {
			...
			if (cnt <= 2)
				alerts.push(`Class ${cls} senate election in ${cnt} turn${cnt === 1 ? '' : 's'}`);
		}
		return alerts;
	});
	...
	async function hydrateSessionState() {
		...
		const turn = await api
			.get<TurnState>(`/api/v1/sessions/${sessionId}/turns/current`)
			.catch((): TurnState | null => null);
		if (turn) setCurrentTurnOnly(turn);

		// Restore national status snapshot.
		const sessionWithStatus = state.session as SessionWithServerFields;
		if (sessionWithStatus.national_status) {
			try {
				const ns =
					typeof sessionWithStatus.national_status === 'string'
						? (JSON.parse(sessionWithStatus.national_status) as NationalStatusPayload)
						: sessionWithStatus.national_status;
	...
	onMount(() => {
		...
		const joinRoom = () => {
			...
			reconnectTimer = setTimeout(() => {
				hydrateSessionState().catch(() => undefined);
			}, 100);
		};
...
{#if myPlayer}
	<div
		style="background:#1c1a14;border-bottom:1px solid #3d3520;padding:8px 16px;font-family:Georgia,serif;font-size:13px;color:#d4b896;display:flex;gap:16px;align-items:center;flex-wrap:wrap;"
	>
		...
		{#each electionAlert() as alert (alert)}
			<span
				style="background:#7a2222;color:#fff;font-size:10px;font-weight:700;padding:2px 7px;border-radius:3px;letter-spacing:.03em;"
			>
				⚑ {alert}
			</span>
		{/each}
...
{:else}
	...
	{#if electionAlert().length > 0}
		<div
			style="background:#1c1a14;border-bottom:1px solid #3d3520;padding:6px 16px;display:flex;gap:8px;flex-wrap:wrap;"
		>
			{#each electionAlert() as alert (alert)}
				<span
					style="background:#7a2222;color:#fff;font-size:10px;font-weight:700;padding:2px 7px;border-radius:3px;"
					>⚑ {alert}</span
				>
			{/each}
		</div>
...
{#if $statNotifications.length > 0}
	<div
		style="position:fixed;bottom:16px;right:16px;z-index:200;display:flex;flex-direction:column-reverse;gap:8px;max-width:340px;pointer-events:none;"
	>
		{#each $statNotifications as n (n.id)}
			<div
				style="background:#232019;color:#faf7f0;border-left:4px solid {n.positive
					? '#3b6d11'
					: '#a32d2d'};border-radius:4px;padding:10px 14px;font-size:12px;font-family:Georgia,serif;box-shadow:0 2px 8px rgba(0,0,0,0.4);"
			>
				{n.text}
			</div>
		{/each}
