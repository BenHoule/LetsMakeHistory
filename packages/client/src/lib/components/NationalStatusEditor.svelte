...
<div class="border rounded p-4 space-y-3">
	...
	<p class="text-xs text-gray-500">
		Set before advancing to Voting. Broadcast to all players and used in player-action prompts.
	</p>
	{#each [['Financial', 'financial'] as const, ['Social', 'social'] as const, ['Foreign', 'foreign'] as const] as [label, key] (key)}
		<div class="space-y-1">
			...
			{#if key === 'financial'}
				<input
					id="national-status-financial"
					class="border rounded w-full px-2 py-1 text-sm"
					bind:value={financial}
					placeholder="e.g. Industrial expansion outpaces wages"
				/>
			{:else if key === 'social'}
				<input
					id="national-status-social"
					class="border rounded w-full px-2 py-1 text-sm"
					bind:value={social}
					placeholder="e.g. Urban labour tensions rising"
				/>
			{:else}
				<input
					id="national-status-foreign"
					class="border rounded w-full px-2 py-1 text-sm"
					bind:value={foreign}
					placeholder="e.g. Naval competition with Europe"
				/>
			{/if}
		</div>
	{/each}
