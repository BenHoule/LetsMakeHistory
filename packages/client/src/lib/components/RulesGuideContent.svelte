...
{#snippet inline(parts: InlinePart[])}
	{#each parts as part, index (`${part.type}-${index}-${part.value}`)}
		...
		{:else if part.type === 'code'}<code
				style="background:#e8e1cc;padding:1px 4px;border-radius:4px;font-size:0.95em;"
				>{part.value}</code
			>
		{:else}{part.value}
		{/if}
...
{:else if error}
	<div
		style="padding:12px 14px;border:1px solid #a32d2d;border-radius:6px;background:#fbe8e8;color:#8a1f1f;font-size:14px;"
	>
		{error}
	</div>
{:else}
	<div>
		{#each blocks as block, index (`${block.type}-${index}`)}
			{#if block.type === 'h1'}
				<h2 style="font-family:Georgia,serif;font-size:28px;margin:28px 0 12px;">
					{@render inline(block.parts)}
				</h2>
			{:else if block.type === 'h2'}
				<h3 style="font-family:Georgia,serif;font-size:20px;margin:24px 0 10px;">
					{@render inline(block.parts)}
				</h3>
						...
						<thead>
							<tr>
								{#each block.headers as cell, cellIndex (`h-${cellIndex}`)}
									<th
										style="text-align:left;padding:8px 10px;border-bottom:1px solid #b8ae8e;background:#e8e1cc;font-size:12px;text-transform:uppercase;letter-spacing:0.04em;color:#7a7362;"
							...
							{#each block.rows as row, rowIndex (`r-${rowIndex}`)}
								<tr>
									{#each row as cell, cellIndex (`c-${rowIndex}-${cellIndex}`)}
										<td style="padding:8px 10px;border-bottom:1px solid #d8d0b8;vertical-align:top;"
