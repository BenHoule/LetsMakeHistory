<script lang="ts">
	...
	const composedTarget = $derived(
		isPlayerStat ? pTarget : isParty ? party : `${region}:${regParty}`
	);
	...
	async function applyChange() {
		...
		if (isRizz && delta < 0) {
			error = 'Rizz is gain-only — use a positive delta.';
			return;
		}
		error = '';
		applying = true;
...
<div class="border rounded p-4 space-y-3">
	...
	<div class="space-y-1">
		...
		<p class="text-xs text-gray-400">
			Small &plusmn;{deltas[0]} &nbsp;&bull;&nbsp; Medium &plusmn;{deltas[1]} &nbsp;&bull;&nbsp; Large
			&plusmn;{deltas[2]}&nbsp; &nbsp;|&nbsp; Floor: {bounds[0]} &nbsp; Ceiling: {#if bounds[1] === Infinity}&infin;{:else}{bounds[1]}{/if}
	...
	<div class="space-y-1">
		...
		{#if isPlayerStat}
			<select id="sc-target" class="border rounded w-full px-2 py-1 text-sm" bind:value={pTarget}>
				...
				{:else}
					{#each $players as p (p.id ?? p.name)}<option value={p.name}>{p.name} ({p.party})</option
						>{/each}
				{/if}
			</select>
		{:else if isParty}
