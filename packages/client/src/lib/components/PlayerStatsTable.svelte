<script lang="ts">
	...
	type EditablePlayerPatch = Partial<
						...
						<td class="py-1 pr-3 text-gray-600">
							{#if editable}
								<select
									...
									disabled={savingPlayerId === p.id}
									onchange={(e) =>
										saveTextField(p, 'region', (e.target as HTMLSelectElement).value)}
									class="w-full rounded border px-1 py-0.5 disabled:bg-gray-100 disabled:text-gray-400"
								>
						...
						<td class="py-1 pr-3 text-right">
							{#if editable}
								<select
									...
									disabled={savingPlayerId === p.id}
									onchange={(e) =>
										saveNumberField(p, 'class', (e.target as HTMLSelectElement).value)}
						...
						<td class="py-1 pr-2 text-right">
							{#if editable}
								<input
									...
									disabled={savingPlayerId === p.id}
									onchange={(e) =>
										saveNumberField(p, 'approval', (e.target as HTMLInputElement).value)}
						...
						<td class="py-1 pr-2 text-right">
							{#if editable}
								<input
									...
									disabled={savingPlayerId === p.id}
									onchange={(e) =>
										saveNumberField(p, 'recognition', (e.target as HTMLInputElement).value)}
									class="w-16 rounded border px-1 py-0.5 text-right disabled:bg-gray-100 disabled:text-gray-400"
								/>
							{:else}
