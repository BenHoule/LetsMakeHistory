      ...
      <tbody>
        {#each $players as p}
          <tr class="border-t">
            ...
            <td class="py-1 pr-3 text-gray-600">{p.region}</td>
            <td class="py-1 pr-2 text-right">{p.approval.toFixed(1)}</td>
            <td class="py-1 pr-2 text-right">{p.recognition.toFixed(0)}</td>
            <td class="py-1 text-right">{p.rizz.toFixed(0)}</td>
          </tr>
        {/each}
      </tbody>
    </table>
