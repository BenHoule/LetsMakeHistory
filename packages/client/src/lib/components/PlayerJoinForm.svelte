<script lang="ts">
	...
	async function loadSeats() {
		seats = await api
			.get<Record<string, Record<number, number>>>(`/api/v1/sessions/${sessionId}/available-seats`)
			.catch(() => ({}));
	...
	async function loadActivePlayers() {
		const state = await api
			.get<SessionStateResponse>(`/api/v1/sessions/${sessionId}`)
			.catch(() => null);
	...
	async function joinSession() {
		...
		try {
			...
			await loadSeats();
			error =
				e instanceof Error ? e.message : 'That seat is no longer available. Please choose another.';
		} finally {
			submitting = false;
		}
	}
...
>
	...
	<select class="border p-2 w-full" bind:value={clss}>
		{#each CLASSES as c (c)}
			<option value={c} disabled={isFull(region, c)}
				>Class {c}{isFull(region, c) ? ' (taken)' : ''}</option
			>
		{/each}
	</select>
						...
						{#each sortedActivePlayers as p (p.id)}
							<tr class="border-b last:border-b-0">
								...
								<td
									...
									style="color:{PARTY_COLORS[p.party as Party] ?? '#555'};">{p.party}</td
								>
