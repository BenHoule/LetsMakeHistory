  ...
  async function loadHistory() {
    ...
    try {
      historyRecords = await api.gmGet<Array<Record<string, unknown>>>(
        `/api/v1/gm/sessions/${sessionId}/actions`, gmToken
      );
    } catch { historyRecords = []; }
    finally { historyLoading = false; }
  }

  function openHistory() { historyOpen = true; loadHistory(); }
      ...
      <div class="border rounded p-3 space-y-2 bg-gray-50">
        ...
        <div class="flex items-center gap-2 text-sm">
          ...
          <span class="ml-auto flex items-center gap-1.5">
            <span class="text-xs text-gray-400">Turn {action.turnIndex + 1} &middot; {action.year}</span>
          </span>
        </div>
        ...
        <div class="space-y-0.5">
          ...
          <textarea
            ...
            oninput={(e) => updateActionEdit(action.actionId, 'editedContent', (e.target as HTMLTextAreaElement).value)}></textarea>
        </div>

        <!-- Claude prompt (collapsible) -->
        <details class="text-xs">
          <summary class="cursor-pointer text-gray-500 hover:text-gray-700">Claude narrative prompt</summary>
          <pre class="mt-2 p-2 bg-white border rounded whitespace-pre-wrap font-mono text-xs leading-relaxed">{action.prompt}</pre>
        </details>
...
{#if historyOpen}
  ...
  <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50"
    ...
    <!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
    <div class="bg-white rounded-lg p-6 w-full max-w-lg mx-4 space-y-3 shadow-xl max-h-[80vh] flex flex-col"
          ...
          {#each historyRecords as r (`${r.actionId ?? ''}-${r.turnIndex ?? ''}-${r.year ?? ''}-${r.playerName ?? ''}`)}
            <div class="border rounded p-2 text-xs space-y-0.5">
              <div class="flex gap-2 font-medium">
                ...
                <span class="ml-auto text-gray-400">T{Number(r.turnIndex ?? 0)+1} &middot; {r.year}</span>
              </div>
