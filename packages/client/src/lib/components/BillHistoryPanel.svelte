<script lang="ts">
	...
	async function load() {
		loading = true;
		entries = await api
			.get<BillEntry[]>(`/api/v1/sessions/${sessionId}/bill-history`)
			.catch(() => []);
		loading = false;
	}
...
<section
	style="background:#1c1a14;border:1px solid #3d3520;border-radius:4px;overflow:hidden;font-family:Georgia,serif;"
>
	<button
		onclick={toggle}
					...
					<thead>
						<tr style="color:#8a7a55;border-bottom:1px solid #3d3520;">
							{#each ['Turn', 'Year', 'Title', 'Type', 'Proposer', 'Yea', 'Nay', 'Abstain', 'Result'] as h (h)}
								<th
									style="text-align:left;padding:3px 5px;font-size:10px;text-transform:uppercase;white-space:nowrap;"
									>{h}</th
								>
							{/each}
						</tr>
					...
					<tbody>
						{#each entries as b (b.id)}
							<tr style="border-bottom:1px solid #2a2518;color:#c8b99a;">
								<td style="padding:3px 5px;color:#7a6a44;"
									>{b.turn_index != null ? b.turn_index + 1 : '—'}</td
								>
								<td style="padding:3px 5px;color:#7a6a44;">{b.year ?? '—'}</td>
								<td
									style="padding:3px 5px;font-weight:600;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
								...
								<td style="padding:3px 5px;font-weight:700;color:{resultColor(b.vote_result)};"
									>{b.vote_result}</td
								>
							</tr>
						{/each}
