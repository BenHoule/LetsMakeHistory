...
{#if $pendingActionVotes.length > 0}
  <div class="space-y-3">
    {#each $pendingActionVotes as vote (vote.actionId)}
      ...
      <div style="border:2px solid #2d5a8a;border-radius:6px;padding:14px;background:#101820;font-family:Georgia,serif;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
          <span style="font-weight:700;color:#6ab4f0;font-size:13px;">Legislative Vote</span>
          <span style="color:#7a9ab8;font-size:12px;">proposed by {vote.playerName} ({vote.party})</span>
        </div>
        ...
        {:else}
          <p style="font-size:12px;font-weight:700;color:{vote.myVote === 'YEA' ? '#5fa33b' : '#c0392b'};margin-bottom:10px;">
...
{#if $voteResults.length > 0}
  <div style="border:1px solid #3d3520;border-radius:4px;padding:10px 12px;background:#1c1a14;font-family:Georgia,serif;font-size:12px;margin-top:4px;">
    <div style="color:#a89060;font-size:11px;font-weight:700;margin-bottom:6px;text-transform:uppercase;letter-spacing:.05em;">Recent Vote Results</div>
    {#each $voteResults.slice(0, 5) as r (r.actionId + r.resolvedAt)}
      <div style="display:flex;gap:8px;padding:3px 0;border-bottom:1px solid #2a2518;color:#c8b99a;">
        <span style="font-weight:700;color:{r.passed?'#5fa33b':'#c0392b'};">{r.passed ? 'PASSED' : 'FAILED'}</span>
